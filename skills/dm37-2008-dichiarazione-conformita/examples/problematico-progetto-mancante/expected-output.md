# Verifica completezza DdC - DM 37/2008 Allegato I

**Data verifica**: 20/03/2026
**DdC analizzata**: Impianto elettrico Corso Vittorio Emanuele 88 p. 2, Bologna
**Categoria impianto**: a) impianto elettrico

## Esito sintetico

**INCOMPLETA - NON CONFORME**

Carenze formali: 4
Allegati obbligatori mancanti: 2 (progetto da professionista + elaborato tecnico qualsiasi)
Carenze qualitative: 2 (formula generica, relazione materiali inadeguata)

## Analisi soglie Art. 5 - Obbligo di progetto da professionista

Categoria: a) impianto elettrico
Caratteristiche: potenza 15 kW, superficie 350 mq, edificio uso uffici (terziario), no ATEX

Verifica soglie Art. 5 DM 37/2008:
- Alimentazione media tensione: NO
- Potenza > 6 kW: **SI** (15 kW >> 6 kW)
- Luoghi ATEX: NO
- Altezza in gronda > 12 m (edificio civile): non dichiarata - da verificare

**Esito**: Progetto da professionista iscritto all'albo **OBBLIGATORIO** - potenza 15 kW supera la soglia di 6 kW (Art. 5 DM 37/2008).

Conseguenza: il progetto redatto da tecnico abilitato (ingegnere elettrico / perito industriale) era obbligatorio prima dei lavori. Lo schema as-built dell'impianto realizzato deve essere allegato insieme al progetto (Art. 7 co. 1 DM 37/2008).

**Nota**: la soglia dei 350 mq in uso terziario non e' una soglia Art. 5 rilevante per cat. a) (le soglie mq riguardano le unita' abitative residenziali). La soglia determinante qui e' la potenza (6 kW).

## Dettaglio per sezione

### Sezione 1 - Dati impresa
- [X] Ragione sociale: presente - "Elettroimpianti SRL"
- [X] Sede legale: presente - Corso Vittorio Emanuele 88, Bologna
- [X] Iscrizione Registro Imprese (Art. 3 DM 37/2008): presente - n. BO-2020-123456
- [X] P.IVA: presente - 09876543210

### Sezione 2 - Responsabile tecnico
- [X] Nome e cognome: presente - Verdi Giuseppe
- [!] Qualifica (Art. 4 DM 37/2008): **INCOMPLETO** - "Diploma ITIS" senza specificare l'indirizzo (elettrico? meccanico?) e senza indicare il percorso di abilitazione (lett. a, b o c Art. 4)
- [!] Abilitazione categoria specifica: **INADEGUATO** - "tecnico abilitato" non specifica la categoria di abilitazione; per impianti elettrici (cat. a) deve essere esplicitamente indicata

### Sezione 3 - Dati impianto
- [X] Categoria impianto: presente - a) impianto elettrico
- [X] Destinazione d'uso: presente - ufficio/terziario
- [X] Indirizzo impianto: presente - Corso Vittorio Emanuele 88 p. 2, Bologna
- [X] Committente: presente - Studio Legale Bianchi e Associati SRL

### Sezione 4 - Dichiarazione e norme
- [!] Formula dichiarativa: **INADEGUATA** - "l'impianto e' stato realizzato secondo le norme vigenti" e' troppo generica e non corrisponde alla formula del modello Allegato I ("in conformita' alla regola dell'arte")
- [!] Norma tecnica specifica: **ASSENTE** - nessuna norma tecnica indicata. "Norme vigenti" non e' una citazione accettabile; per impianti cat. a) va indicata almeno la CEI 64-8
- [X] Data: presente - 20/03/2026
- [X] Firma: presente - Giuseppe Verdi

### Sezione 5 - Allegati
- [!] Relazione materiali (OBBLIGATORIA - Art. 7 co. 1): presente ma **INADEGUATA** - "cavi conformi alle norme CEI, quadro certificato" e' generica; non specifica tipologia, sezione, marca/modello
- [!] Elaborato tecnico (schema/progetto): **ASSENTE - NON CONFORME** - nessuno schema ne' progetto allegato; obbligatorio in tutti i casi (Art. 7 co. 1 DM 37/2008)
- [!] Progetto da professionista: **ASSENTE - NON CONFORME** - obbligatorio (potenza 15 kW > 6 kW, Art. 5 DM 37/2008)

## Carenze rilevate

| # | Sezione | Campo | Carenza | Riferimento | Priorita' |
|---|---------|-------|---------|-------------|-----------|
| 1 | 5 | Progetto da professionista | Mancante - obbligatorio per potenza 15 kW > 6 kW | Art. 5 + Art. 7 co. 1 DM 37/2008 | **CRITICA** |
| 2 | 5 | Schema impianto | Nessun elaborato tecnico - obbligatorio in tutti i casi | Art. 7 co. 1-2 DM 37/2008 | **CRITICA** |
| 3 | 4 | Norma tecnica | Nessuna norma tecnica specifica citata | Allegato I DM 37/2008 | Alta |
| 4 | 4 | Formula dichiarativa | Generica, non conforme al modello Allegato I | Allegato I DM 37/2008 | Alta |
| 5 | 2 | Abilitazione | Categoria abilitazione non specificata | Art. 7 co. 1 + Art. 4 DM 37/2008 | Media |
| 6 | 5 | Relazione materiali | Descrizione generica senza tipologia/marca/modello | Art. 7 co. 1 DM 37/2008 | Media |

## Raccomandazioni

1. **Urgente**: Carenza critica - il progetto da tecnico abilitato era obbligatorio prima dell'inizio dei lavori (Art. 5 DM 37/2008). Far valutare da un ingegnere elettrico / perito industriale iscritto all'albo il percorso documentale corretto, considerando lo stato attuale dell'impianto. La redazione retroattiva di un "progetto as-built" non equivale al progetto preventivo obbligatorio per legge: la questione deve essere affrontata con il professionista competente.
2. **Urgente**: Allegare lo schema dell'impianto realizzato (as-built) redatto dal responsabile tecnico.
3. Correggere la formula dichiarativa con quella conforme all'Allegato I DM 37/2008.
4. Indicare esplicitamente la norma tecnica applicata: per impianti elettrici cat. a) la CEI 64-8 (specificare edizione).
5. Specificare nella DdC la categoria di abilitazione del responsabile tecnico e il percorso (lett. a, b o c Art. 4).
6. Integrare la relazione materiali: tipologia cavi (sezione, tensione nominale), marca/modello quadro e interruttori.

## Limiti di questa verifica

- Verifica formale; non giudica se il progetto (che manca) sarebbe tecnicamente adeguato
- Non verifica la coerenza tra impianto realizzato e quanto dichiarato nella DdC
- La soglia Art. 5 e' verificata sulla potenza dichiarata (15 kW); se la potenza reale fosse diversa, la valutazione cambia
- Le soglie Art. 5 devono essere verificate su Normattiva per uso operativo

## Rinvio al responsabile tecnico firmatario

**La presente verifica non sostituisce la revisione e firma del responsabile tecnico abilitato.**
Questa DdC, nello stato attuale, presenta carenze critiche che devono essere risolte prima della consegna al committente. La responsabilita' penale e civile per la DdC resta in capo all'installatore firmatario (Art. 7 co. 4 DM 37/2008).

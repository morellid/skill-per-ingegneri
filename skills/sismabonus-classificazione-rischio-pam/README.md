...
Skill code-driven per il calcolo della classe di rischio sismico di un edificio esistente ai sensi del DM 58/2017 Allegato A (sostituito dal DM 65/2017 - testo letterale di riferimento; il DM 329/2020 modifica art. 3 e Allegato B, NON l'Allegato A), metodo CONVENZIONALE: PAM (con capping prescritto al passo 2.1.3) + IS-V -> classe finale + salto classi tra stato di fatto e stato di progetto.
...
- Applica di default il capping prescritto dall'Allegato A passo 2.1.3 (TR_C(SLO/SLD) := min(TR_C(SLO/SLD), TR_C(SLV))). Disattivabile via `--no-capping` per validazione vs software che applicano il capping a monte.
- Calcola l'IS-V (Indice di Sicurezza per la Vita) = PGA_C(SLV) / PGA_D(SLV).
- Attribuisce la classe PAM (8 classi A+..G, Tab. 1 dell'Allegato A) e la classe IS-V (7 classi A+..F, Tab. 2).
- Determina la classe finale come la peggiore tra le due.
- Confronta stato di fatto e stato di progetto e calcola il salto classi.
- Segnala warning sulle curve di individuazione non monotone residue dopo il capping.
- Verificato numericamente vs il valore PAM = 1,13% dichiarato dal decreto stesso (Allegato A nota a Tab. 1) per V_R=50 ai minimi NTC.
...
Fonti primarie (tutte pubbliche, fetched da PDF MIT ufficiali):
- DM Min. Infrastrutture e Trasporti 28/2/2017 n. 58 - decreto istitutivo
- DM MIT 7/3/2017 n. 65 - **sostituisce l'Allegato A** del DM 58/2017 (testo letterale di riferimento per la procedura di classificazione)
- DM MIT 9/1/2020 n. 24 - aggiornamento successivo
- DM MIT 6/8/2020 n. 329 - **modifica art. 3** del DM 58/2017 (super-sismabonus 110%) e **sostituisce l'Allegato B** (asseverazione tecnica). Non modifica l'Allegato A.

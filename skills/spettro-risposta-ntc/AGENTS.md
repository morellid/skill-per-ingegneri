...
- Validazione: Livello 1 self-check OK (`./scripts/validate.sh spettro-risposta-ntc`); test suite Python: 36/36 pass (12 classi: formule chiuse, raccordi, interpolazione, validazione input hardening, copertura categorie D/E/T2/T3/T4 e xi != 5%, anti-drift fixture-based su `examples/caso-conforme-.../{input,expected}.json`, CLI). Livello 2 (confronto vs foglio CSLP su 10+ casi reali) da completare prima del release stabile.
- Task files:

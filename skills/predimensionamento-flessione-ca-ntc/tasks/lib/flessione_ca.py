...
ES_DEFAULT = 200000.0          # MPa, modulo elastico acciaio (valore standard EN 1992-1-1, NTC par. 11.3.2.1)
EPS_CU_DEFAULT = 0.0035        # 3.5 per mille, fck <= 50 MPa Classe 1 (NTC 2018 par. 4.1.2.1.2.1)
LAMBDA_SB = 0.8                # stress-block: profondita' equivalente (EN 1992-1-1 par. 3.1.7, Classe 1)
ETA_SB = 1.0                   # stress-block: tensione costante, eta*fcd (EN 1992-1-1 par. 3.1.7, Classe 1)
FCK_MAX_CLASSE_1 = 50.0        # MPa, C50/60 max per lambda=0.8 eta=1 (NTC 2018 par. 4.1.2.1.2.1)
ZETA_LIM_DUTTILITA = 0.45      # x/d <= 0.45 per fck<=50 MPa (NTC 2018 par. 4.1.1.1, ridistrib. momenti)
...
def resistenza_calcestruzzo_fcd(
    ...
    """fcd = alpha_cc * fck / gamma_c (NTC 2018 eq. [4.1.3], par. 4.1.2.1.1.1)."""
    fck = _positive(_finite(fck, "fck"), "fck")
    if fck > FCK_MAX_CLASSE_1:
        raise ValueError(
            f"fck = {fck} MPa > {FCK_MAX_CLASSE_1} MPa: la skill copre solo Classe 1 (fck <= 50 MPa, "
            "fino a C50/60). Per fck > 50 MPa il modello stress-block rettangolare richiede "
            "lambda < 0,8 e eta < 1 (EN 1992-1-1 par. 3.1.7(3), recepito da NTC 2018 par. "
            "4.1.2.1.2.1): fuori scope skill v0.1."
        )
...
def calcola_m_rd(
    ...
    Riferimenti (verificati su PDF, hash SHA256 in references/sources.yaml):
      - NTC 2018 par. 4.1.2.1.1.1: fcd = alpha_cc * fck / gamma_c (eq. [4.1.3])
      - NTC 2018 par. 4.1.2.1.1.3: fyd = fyk / gamma_s (eq. [4.1.5])
      - NTC 2018 par. 4.1.2.1.2.1: eps_cu = 0,0035 per Classe 1; modello stress-block (Fig. 4.1.1(c))
      - NTC 2018 par. 4.1.2.3.4.1: ipotesi sezioni piane, trazione cls nulla
      - NTC 2018 par. 4.1.2.3.4.2: MRd >= MEd (eq. [4.1.18a])
      - NTC 2018 par. 4.1.1.1: limite x/d <= 0,45 per ridistribuzione momenti
      - Circ. MIT n. 7/2019 par. C4.1.2.3.4.2: duttilita' curvatura richiesta solo Cap. 7
    """
    b = _positive(_finite(b, "b"), "b")
    ...
    if not duttile_zeta:
        avvertenze.append(
            f"x/d = {zeta:.3f} > {ZETA_LIM_DUTTILITA} (limite per ridistribuzione momenti "
            "nelle travi continue fck<=50 MPa, NTC 2018 par. 4.1.1.1): la sezione e' poco "
            "duttile. Per casi non sismici il calcolo MRd resta valido finche' eps_s >= eps_yd, "
            "ma il progettista dovrebbe verificare la necessita' di ridistribuzione. "
            "In zona sismica valgono i limiti piu' stringenti di NTC par. 7.4. "
            "Aumentare b o h, oppure passare a sezione doppiamente armata."
        )

    riferimenti = [
        "NTC 2018 par. 4.1.2.1.1.1 - fcd = alpha_cc*fck/gamma_c (eq. [4.1.3])",
        "NTC 2018 par. 4.1.2.1.1.3 - fyd = fyk/gamma_s (eq. [4.1.5]); gamma_s = 1,15",
        "NTC 2018 par. 4.1.2.1.2.1 - stress-block: eps_cu=0,0035 (Classe 1); lambda=0,8 eta=1 (EN 1992-1-1)",
        "NTC 2018 par. 4.1.2.3.4.1 - ipotesi sezioni piane, trazione cls nulla",
        "NTC 2018 par. 4.1.2.3.4.2 - MRd >= MEd (eq. [4.1.18a]); formula MRd da stress-block",
        "NTC 2018 par. 4.1.1.1 - limite x/d <= 0,45 per ridistribuzione momenti (travi continue)",
        "Circ. MIT n. 7/2019 par. C4.1.2.3.4.2 - duttilita' di curvatura richiesta solo al Cap. 7 (sismica)",
    ]

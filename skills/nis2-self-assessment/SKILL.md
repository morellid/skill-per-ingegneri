...
Se la richiesta e' generica ("aiutami con NIS2"), chiedi all'utente quale delle quattro azioni vuole svolgere. La sequenza logica naturale e': prima `valuta-perimetro`, poi (se in ambito) `gap-analysis-misure` + `check-obblighi-governance`, infine `verifica-incidente-significativo` quando occorre.
